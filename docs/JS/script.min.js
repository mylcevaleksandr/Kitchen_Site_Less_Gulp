...
if(!popupPhone.value){popupPhone.style.borderColor="red";popupPhone.nextElementSibling.style.display="flex";}else{loader.style.display="flex";$.ajax({method:"POST",url:"http://testologia.site/checkout",data:{name:popupName.value}}).done(function(msg){if(msg.success===1){alert("Возникла ошибка при оформлении заказа");loader.style.display="none";location.reload();}else{popupForm.style.display="none";popupSuccess.style.display="flex";loader.style.display="none";setTimeout(function(){location.reload();},5000);}}).fail((jqXHR,textStatus)=>{alert("AJAX request failed: "+textStatus);loader.style.display="none";});}});formBtn.addEventListener("click",()=>{for(let item of errorMsg){item.style.display="none";}
...
if(!formDate.value){formDate.style.borderColor="red";formDate.nextElementSibling.style.display="flex";}else{loader.style.display="flex";$.ajax({method:"POST",url:"http://testologia.site/checkout",data:{name:formName.value}}).done((msg)=>{if(msg.success===1){alert("Возникла ошибка при оформлении заказа");loader.style.display="none";location.reload();}else{form.style.display="none";formSuccess.style.display="flex";loader.style.display="none";setTimeout(function(){location.reload();},5000);}}).fail((jqXHR,textStatus)=>{alert("AJAX request failed: "+textStatus);loader.style.display="none";});}});const getIndex=()=>{let result;indicators.forEach(el=>{if(el.classList.contains("active")){result=el.dataset.slideTo;}});return result;};const changeInfo=(index)=>{infoSlides.forEach((el,i)=>{el.classList.remove("active");if(i===index){el.classList.add("active");}});};const observer=new MutationObserver((mutationsList)=>{for(const mutation of mutationsList){if(mutation.type==="attributes"&&mutation.attributeName==="class"){changeInfo(getIndex());}}});const config={attributes:true,childList:false,subtree:false};indicators.forEach(el=>{observer.observe(el,config);});